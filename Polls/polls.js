...
function createPollElement(poll, pollId, polls, index) {
  ...
  const hue = (poll.userName.charCodeAt(0) * 137.508) % 200; // Use the character code of the first letter for better distribution
  const adjustedHue = (hue + 200) % 360; // Shift hue towards the blue-purple range
  profileCircle.style.backgroundColor = `hsl(${adjustedHue}, 70%, 70%)`; // Keep saturation and lightness constant
